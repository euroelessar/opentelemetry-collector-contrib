...
import (
	...
	"github.com/euroelessar/opentelemetry-collector-contrib/processor/deltatocumulativeprocessor/internal/data/expo/expotest"
)
...
// generateBinsWithZeros creates a bins slice with leading and trailing zeros.
func generateBinsWithZeros(offset, leadingZeros, middleLength, trailingZeros int, value uint64) expo.Buckets {
	buckets := pmetric.NewExponentialHistogramDataPointBuckets()
	buckets.SetOffset(int32(offset - leadingZeros))
	totalLength := leadingZeros + middleLength + trailingZeros
	counts := make([]uint64, totalLength)
	for i := leadingZeros; i < leadingZeros+middleLength; i++ {
		counts[i] = value
	}
	buckets.BucketCounts().FromRaw(counts)
	return buckets
...
// generateRawBins creates a raw bins slice with specified length, offset, and value.
func generateRawBins(length int, value uint64) []uint64 {
